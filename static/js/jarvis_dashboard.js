...
function renderHeroSummaryPanel(data, justRefreshed = false) {
  ...
  const topSource = (data.articles && data.articles.length > 0) ? data.articles[0].source_name : 'Global AI Sources';
  ...
  container.innerHTML = `
      ...
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px; flex-wrap: wrap; gap: 10px;">
        ...
        <div style="display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
          <div style="font-size: 0.85rem; color: var(--text-muted); font-family: var(--font-code);">
            Date: <strong style="color: var(--accent-cyan);">${data.date}</strong>
          </div>
          <button class="btn btn-outline" style="padding: 6px 14px; font-size: 0.82rem; border-color: var(--accent-cyan); color: var(--accent-cyan); font-weight: 700;" onclick="initJarvisDashboard(true)">
            <i class="fas fa-sync-alt"></i> Refresh Live News
          </button>
        </div>
      </div>
      
      <p style="font-size: 1.02rem; color: var(--text-main); line-height: 1.7; margin-bottom: 0;">
        "${data.daily_summary || "Today's real-time AI developments summarized daily by Jarvis AI Agent."}"
      </p>
    </div>
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 14px; margin-bottom: 36px;">
      <div class="glass-card" style="padding: 14px 18px; border-radius: 16px; text-align: center; border: 1px solid var(--border-glass);">
        <div style="font-size: 0.75rem; color: var(--text-dim); text-transform: uppercase; margin-bottom: 4px;">Update Status</div>
        <div style="font-size: 0.92rem; font-weight: 700; color: var(--accent-cyan); font-family: var(--font-code);">${data.date}</div>
      </div>
      ...
      <div class="glass-card" style="padding: 14px 18px; border-radius: 16px; text-align: center; border: 1px solid var(--border-glass);">
        <div style="font-size: 0.75rem; color: var(--text-dim); text-transform: uppercase; margin-bottom: 4px;">Top Outlet</div>
      ...
      <div class="glass-card" style="padding: 14px 18px; border-radius: 16px; text-align: center; border: 1px solid var(--border-glass);">
        <div style="font-size: 0.75rem; color: var(--text-dim); text-transform: uppercase; margin-bottom: 4px;">Frequency</div>
        <div style="font-size: 0.85rem; font-weight: 700; color: var(--accent-green); display: flex; align-items: center; justify-content: center; gap: 6px;">
          <i class="fas fa-circle" style="font-size: 0.4rem; color: #00e676;"></i> Updated Daily
        </div>
      </div>
    </div>
  `;
